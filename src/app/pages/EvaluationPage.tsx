...
// Trend Chart — All dimensions over 12 weeks
function TrendChart({ activeDimensions, setActiveDimensions }: { activeDimensions: string[]; setActiveDimensions: (d: string[]) => void }) {
  const toggleDim = (dim: string) => {
    if (activeDimensions.includes(dim)) {
      if (activeDimensions.length > 1) setActiveDimensions(activeDimensions.filter((d) => d !== dim));
    } else {
  ...
  return (
    <div>
      <SectionLabel number="01" en="Longitudinal Trend" zh="各核心维度阶段趋势分析" />

      {/* Dimension selector */}
      <div style={{ display: 'flex', gap: '8px', marginBottom: '24px', flexWrap: 'wrap' }}>
        {Object.entries(DIMENSION_LABELS).map(([key, label]) => {
          const active = activeDimensions.includes(key);
          return (
            <button
              key={key}
              ...
              style={{
                padding: '6px 14px',
                border: `1px solid ${active ? DIMENSION_COLORS[key as keyof typeof DIMENSION_COLORS] : '#DED8CC'}`,
                borderRadius: '2px',
                backgroundColor: active ? `${DIMENSION_COLORS[key as keyof typeof DIMENSION_COLORS]}18` : 'transparent',
                color: active ? DIMENSION_COLORS[key as keyof typeof DIMENSION_COLORS] : '#7A7068',
                fontSize: '12px',
                cursor: 'pointer',
                transition: 'all 0.2s',
                display: 'flex',
            ...
            >
              <span
                style={{
                  ...
                  height: '8px',
                  borderRadius: '50%',
                  backgroundColor: active ? DIMENSION_COLORS[key as keyof typeof DIMENSION_COLORS] : '#DED8CC',
      ...
      <ResponsiveContainer width="100%" height={360}>
        <LineChart data={WEEKLY_TRENDS} margin={{ top: 8, right: 16, bottom: 8, left: -20 }}>
          ...
          {Object.entries(DIMENSION_COLORS).map(([key, color]) =>
            activeDimensions.includes(key) ? (
              <Line
                ...
                stroke={color}
                strokeWidth={2}
                dot={{ r: 4, fill: color, strokeWidth: 2, stroke: '#F4F0E8' }}
      ...
      >
        <span>阶段起点：<span style={{ fontFamily: "'Space Mono', monospace", color: '#2C5F8A' }}>2.40</span></span>
        <span>阶段终点：<span style={{ fontFamily: "'Space Mono', monospace", color: '#4E7D6A' }}>3.62</span></span>
        <span>综合变化：<span style={{ fontFamily: "'Space Mono', monospace", color: '#C07830' }}>+50% 左右</span></span>
      </div>
    </div>
  );
}
